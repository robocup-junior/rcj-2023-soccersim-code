...
class MyRobot1(RCJSoccerRobot):
    ...
    def sos_placer(self):
        if r_1.role == Role.GOLDAN.value:
            r_1.place = 1
            if self.dist_cal([r_2.x, r_2.y], [ball_1.x, ball_1.y]) > self.dist_cal([r_3.x, r_3.y], [ball_1.x, ball_1.y]):
                r_2.place = 3
                r_3.place = 2
            else:
                r_2.place = 2
                r_3.place = 3
        elif r_2.role == Role.GOLDAN.value:
            r_2.place = 1
            if self.dist_cal([r_1.x, r_1.y], [ball_1.x, ball_1.y]) > self.dist_cal([r_3.x, r_3.y], [ball_1.x, ball_1.y]):
                r_1.place = 3
                r_3.place = 2
            else:
                r_1.place = 2
            ...
            r_3.place = 1
            if self.dist_cal([r_1.x, r_1.y], [ball_1.x, ball_1.y]) > self.dist_cal([r_2.x, r_2.y], [ball_1.x, ball_1.y]):
                r_1.place = 3
                r_2.place = 2
            else:
                r_1.place = 2
    ...
    def avengers_assemble(self):
        fasele = self.robots_dists([0, 0.7])

        if r_1.role == Role.GOLDAN.value and any(abs(f - fasele[i]) < 0.2 for i, f in enumerate(fasele) if i != 0) and r_1.y > 0.5:
            self.chosen_one(r_1, r_2, r_3, ball_1, ball_2, ball_3)
        elif r_2.role == Role.GOLDAN.value and any(abs(f - fasele[i]) < 0.2 for i, f in enumerate(fasele) if i != 1) and r_2.y > 0.5:
            self.chosen_one(r_2, r_1, r_3, ball_2, ball_1, ball_3)
        elif r_3.role == Role.GOLDAN.value and any(abs(f - fasele[i]) < 0.2 for i, f in enumerate(fasele) if i != 2) and r_3.y > 0.5:
            self.chosen_one(r_3, r_1, r_2, ball_3, ball_1, ball_2)
        elif fasele[0] <= fasele[1] and fasele[0] <= fasele[2]:
            self.chosen_one(r_1, r_2, r_3, ball_1, ball_2, ball_3)
        elif fasele[1] <= fasele[2]:
            self.chosen_one(r_2, r_1, r_3, ball_2, ball_1, ball_3)
        else:
            self.chosen_one(r_3, r_1, r_2, ball_3, ball_1, ball_2)

    def chosen_one(self, robot_1: Robot, robot_2: Robot, robot_3: Robot, toop_1: Ball, toop_2: Ball, toop_3: Ball):
        robot_1.role = Role.GOLDAN.value
        if toop_2.c_ball == False and toop_3.c_ball == False:
            robot_2.role, robot_3.role = Role.FRED.value, Role.FRED.value
        elif toop_2.strength > toop_3.strength:  
            robot_2.role, robot_3.role = Role.ATTACKER.value, Role.FRED.value
        else:
            robot_3.role, robot_2.role = Role.ATTACKER.value, Role.FRED.value
