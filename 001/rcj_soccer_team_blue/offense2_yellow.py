...
class OffenseYellow2:
    ...
    def run_program(self, robot, data, lop1, lop2, roles, trap_stage):
        ...
        if detect and not data.robot_flipped:
            ...
            if self.trap_stage != -1 or (
                    offense1_y < -0.67 and offense2_y < -0.67 and ball_y < -0.7 and offense2_x < ball_x < offense1_x and ball_x > 0.2 and offense1_y > ball_y and offense2_y > ball_y and 0.01 < OffenseYellow2.getDistance(
                    self, offense1_x, offense1_y, offense2_x, offense2_y) < 0.16):
                self.mode = trap_ball_main_mode
                if offense1_y < -0.67 and offense2_y < -0.67 and ball_y < -0.7 and offense2_x < ball_x < offense1_x and ball_x > 0.2 and offense1_y > ball_y and offense2_y > ball_y and 0.01 < OffenseYellow2.getDistance(
                ...
                if OffenseYellow2.rangeAngle(self, offense1_angle, 55, angle_range, 35, 20) and -0.42 < ball_x < -0.12 and ball_y > 0.65 and \
        ...
        if self.mode == goal_attack_mode:
            if OffenseYellow2.rangePosition(self, ball_x, ball_y, 0, 0, 0.1, 0.1) and ball_distance < 0.15 and robot_y < ball_y and OffenseYellow2.getDistance(self, ball_x, ball_y, self.last_ball_x, self.last_ball_y) < math.pow(10, -5) * 5:
            ...
            closest_angle, closest_difference = OffenseYellow2.direction(self, ball_pred_angle, 0, 180)
            ...
            if OffenseYellow2.rangePosition(self, ball_x, ball_y, 0.575, 0.73, 0.01, 0.01) and ball_distance < 0.07 and ball_y > robot_y:
                shoot_angle = 300
                closest_angle, closest_difference = OffenseYellow2.direction(self, robot_angle, (shoot_angle + 360) % 360,
                                                                       (shoot_angle + 360 + 180) % 360)
                if not OffenseYellow2.rangeAngle(self, robot_angle, closest_angle, angle_range):
                    spin_speed = 6
                    left_speed, right_speed = OffenseYellow2.spin(self, left_speed, right_speed, robot_angle, closest_angle,
                                                            spin_speed)
            elif -0.58 < ball_x < 0.58 and 0.675 > ball_y > -0.2:
                left_speed, right_speed = OffenseYellow2.moveCurve(self, move_angle, self.side, 45)
            elif (ball_x < -0.58 or ball_x > 0.58) and ball_y < 0:
                left_speed, right_speed = OffenseYellow2.moveCurve(self, move_angle, self.side, 90)
            else:
                left_speed, right_speed = OffenseYellow2.moveCurve(self, move_angle, self.side, 60)
        ...
        # Robot Ambushing
            ...
            if OffenseYellow2.getDistance(self, ball_x, ball_y, self.last_ball_x, self.last_ball_y) < math.pow(10, -5):
                dest_x, dest_y = point_x, ball_y
                offset = -0.0725
            else:
                dest_x, dest_y = OffenseYellow2.findAttackPoint(self, point_x, ball_x, ball_y, self.last_ball_x, self.last_ball_y)
            ...
            if not OffenseYellow2.rangePosition(self, robot_x, robot_y, dest_x, dest_y, 0.005, 0.005):
                robot_radius = 0.0475
                ball_radius = 0.021
                if ball_distance < 0.1 and abs(robot_x) + robot_radius > abs(ball_x) - ball_radius > abs(dest_x) and \
                    ...
                    temp_dest_x, temp_dest_y = OffenseYellow2.findPoint(self, point_x, robot_x, robot_y, ball_x, ball_y + 0.1)
                    left_speed, right_speed = OffenseYellow2.toPoint(self, left_speed, right_speed, temp_dest_x, temp_dest_y, robot_x,
                                                               robot_y, robot_angle, spin_speed, angle_range)
                else:
                    left_speed, right_speed = OffenseYellow2.toPoint(self, left_speed, right_speed, dest_x, dest_y, robot_x,
                                                           robot_y, robot_angle, spin_speed, angle_range)
                left_speed, right_speed = OffenseYellow2.toPoint(self, left_speed, right_speed, dest_x, dest_y, robot_x,
                                                           robot_y, robot_angle, spin_speed, angle_range)
            else:
                closest_angle, closest_difference = OffenseYellow2.direction(self, robot_angle, (shoot_angle + 360) % 360, (shoot_angle + 360 + 180) % 360)
                if not OffenseYellow2.rangeAngle(self, robot_angle, closest_angle, angle_range):
                    left_speed, right_speed = OffenseYellow2.spin(self, left_speed, right_speed, robot_angle, closest_angle, spin_speed)
                else:
                    left_speed, right_speed = OffenseYellow2.moveStraight(self, robot_angle, 0, 2)
        ...
        # Robot Stationing and Tracking
            ...
            if not detect:
                ...
                if ball_y > 0:
                    if ball_x < 0:
                        attack_point_distance = OffenseYellow2.getDistance(self, ball_x, ball_y, -0.3, 0.3)
                        defense_point_distance = OffenseYellow2.getDistance(self, ball_x, ball_y, -0.3, -0.3)
                    else:
                        attack_point_distance = OffenseYellow2.getDistance(self, ball_x, ball_y, 0.3, 0.3)
                        defense_point_distance = OffenseYellow2.getDistance(self, ball_x, ball_y, 0.3, -0.3)
                    ...
                    dest_x, dest_y = 0, -0.2
            angle_range = 8
            if not OffenseYellow2.rangePosition(self, robot_x, robot_y, dest_x, dest_y, 0.02, 0.02):
                left_speed, right_speed = OffenseYellow2.toPoint(self, left_speed, right_speed, dest_x, dest_y, robot_x,
                                                           robot_y, robot_angle, spin_speed, angle_range)
            else:
                closest_angle, closest_difference = OffenseYellow2.direction(self, robot_angle, 0, 180)
                if not OffenseYellow2.rangeAngle(self, robot_angle, closest_angle, angle_range):
                    left_speed, right_speed = OffenseYellow2.spin(self, left_speed, right_speed, robot_angle, closest_angle, spin_speed)
                else:
                    left_speed, right_speed = OffenseYellow2.moveStraight(self, robot_angle, 0, 2)
        ...
        # Double Pushing
            ...
            if OffenseYellow2.rangePosition(self, robot_x, robot_y, dest_x, dest_y, 0.05, 0.05) or \
                    OffenseYellow2.rangePosition(self, robot_x, robot_y, offense1_x, offense1_y, 0.1,
                                           0.1) and robot_x < offense1_x:
                closest_angle, closest_difference = OffenseYellow2.direction(self, robot_angle, 55, 235)
                if not OffenseYellow2.rangeAngle(self, robot_angle, closest_angle, angle_range):
                    left_speed, right_speed = OffenseYellow2.spin(self, left_speed, right_speed, robot_angle, closest_angle,
                                                            spin_speed)
                else:
                    left_speed, right_speed = OffenseYellow2.moveStraight(self, robot_angle, 55, -10)
            else:
                ...
                point_distance = OffenseYellow2.getDistance(self, robot_x, robot_y, dest_x, dest_y)
                ...
                closest_angle, closest_difference = OffenseYellow2.direction(self, (robot_angle + 180) % 360, 0, 180)
        ...
        # Wall Defense
            ...
            angle_range = 15
            if not OffenseYellow2.rangePosition(self, robot_x, robot_y, dest_x, dest_y, 1, 1, 0.02, 0.4, 0.05, 1):
                point_angle = (math.degrees(math.atan2(dest_x - robot_x, dest_y - robot_y)) - robot_angle + 360) % 360
                ...
                closest_angle, closest_difference = OffenseYellow2.direction(self, point_angle, 0, 180)
                ...
                if not OffenseYellow2.rangeAngle(self, robot_angle, closest_angle, angle_range):
                    ...
                    left_speed, right_speed = OffenseYellow2.moveStraight(self, robot_angle, 0, 10)
        ...
        # Midfield Defense
            ...
            if not OffenseYellow2.rangePosition(self, robot_x, robot_y, dest_x, dest_y, 0.02, 0.02):
                ...
                closest_angle, closest_difference = OffenseYellow2.direction(self, robot_angle, 90, 270)
                if not OffenseYellow2.rangeAngle(self, robot_angle, closest_angle, angle_range):
                    left_speed, right_speed = OffenseYellow2.spin(self, left_speed, right_speed, robot_angle,
                                                            closest_angle, spin_speed)
                else:
                    left_speed, right_speed = OffenseYellow2.moveStraight(self, robot_angle, 90, 2)
        ...
        # Goal Defense
            ...
            if not OffenseYellow2.rangePosition(self, robot_x, robot_y, dest_x, dest_y, 0.02, 0.02):
                ...
                if not OffenseYellow2.rangeAngle(self, robot_angle, closest_angle, angle_range):
                    ...
                    left_speed, right_speed = OffenseYellow2.moveStraight(self, robot_angle, 90, 10)
        ...
        # Main Trap Ball
            ...
            if self.trap_stage == 1:
                if ball_y > -0.7 or not offense2_x < ball_x < offense1_x or ball_distance > 0.1 or offense1_ball_distance > 0.1:
                    ...
                    shoot_angle = 140
                    closest_angle, closest_difference = OffenseYellow2.direction(self, robot_angle,
                                                                               (shoot_angle + 360) % 360,
                                                                               (shoot_angle + 360 + 180) % 360)
                    if not OffenseYellow2.rangeAngle(self, robot_angle, closest_angle, angle_range):
                        left_speed, right_speed = OffenseYellow2.spin(self, left_speed, right_speed, robot_angle,
                        ...
                        move_speed = 0
                        left_speed, right_speed = OffenseYellow2.moveStraight(self, robot_angle, 0, move_speed)

                        if OffenseYellow2.rangePosition(self, offense1_x, offense1_y, offense1_x, -0.67, 0.01, 0.01) and \
                                OffenseYellow2.rangeAngle(self, robot_angle, closest_angle, angle_range):
                            self.trap_stage = 2
                ...
                if OffenseYellow2.rangePosition(self, ball_x, ball_y, 0.56, -0.73, 0.01, 0.01):
                    self.trap_stage = 3
                ...
                if ball_y > -0.51 or ball_x < 0.43:
                    ...
                    if not OffenseYellow2.rangePosition(self, robot_x, robot_y, dest_x, dest_y, 0.005, 0.005):
                        left_speed, right_speed = OffenseYellow2.toPoint(self, left_speed, right_speed, dest_x, dest_y,
                                                                       robot_x,
                                                                       robot_y, robot_angle, spin_speed, angle_range)
                    else:
                        closest_angle, closest_difference = OffenseYellow2.direction(self, robot_angle,
                        ...
                        if not OffenseYellow2.rangeAngle(self, robot_angle, closest_angle, angle_range):
                            ...
                            left_speed, right_speed = OffenseYellow2.moveStraight(self, robot_angle, 0, 0)

                            shoot_angle = 0
                            dest_x, dest_y = 0.5715, -0.595
                            closest_angle, closest_difference = OffenseYellow2.direction(self, offense1_angle,
                                                                                       (shoot_angle + 360) % 360,
                                                                                       (shoot_angle + 360 + 180) % 360)
                            if OffenseYellow2.rangePosition(self, offense1_x, offense1_y, dest_x, dest_y, 0.01, 0.01) and \
                                    OffenseYellow2.rangeAngle(self, offense1_angle, closest_angle, angle_range):
                                self.trap_stage = 4
                ...
                if ball_y > -0.51 or ball_x < 0.43:
                    ...
                    if not OffenseYellow2.rangePosition(self, ball_x, ball_y, 0.63, -0.64, 0.01, 0.01):
                        ...
                        if OffenseYellow2.rangePosition(self, ball_x, ball_y, 0.58, -0.73, 0.01, 0.01) and \
                                OffenseYellow2.getDistance(
                                    self, ball_x, ball_y, self.last_ball_x, self.last_ball_y) < math.pow(10, -5):
                            self.trap_stage = 3
                        elif OffenseYellow2.rangePosition(self, ball_x, ball_y, 0.63, -0.68, 0.01, 0.01) and \
                                OffenseYellow2.getDistance(
                                    self, ball_x, ball_y, self.last_ball_x, self.last_ball_y) < math.pow(10, -5):
                            self.trap_stage = 5

                    elif OffenseYellow2.rangePosition(self, robot_x, robot_y, 0.57, -0.69, 0.01, 0.01):
                        self.trap_stage = 5
                ...
                if ball_x < 0.56 or ball_distance > 0.1 or offense1_ball_distance > 0.1:
                    ...
                    if OffenseYellow2.rangePosition(self, ball_x, ball_y, 0.625, 0.67, 0.01, 0.01):
                        self.trap_stage = 7
                ...
                if ball_x < 0.4 or ball_y < 0.55:
                    ...
                    if not OffenseYellow2.rangePosition(self, ball_x, ball_y, 0.54, 0.73, 0.01, 0.01):
                        dest_x, dest_y = ball_x, ball_y
                        point_angle = (math.degrees(
                            math.atan2(dest_x - robot_x, dest_y - robot_y)) - robot_angle + 360) % 360
                        ...
                        if OffenseYellow2.rangePosition(self, ball_x, ball_y, 0.63, 0.68, 0.01,
                                                      0.01) and OffenseYellow2.getDistance(
                            self, ball_x, ball_y, self.last_ball_x, self.last_ball_y) < math.pow(10, -5):
                            self.trap_stage = 7
                        elif OffenseYellow2.rangePosition(self, ball_x, ball_y, 0.58, 0.73, 0.01, 0.01) and \
                                OffenseYellow2.getDistance(
                                    self, ball_x, ball_y, self.last_ball_x, self.last_ball_y) < math.pow(10,
                                                                                                         -5) and ball_distance < 0.05:
                            self.trap_stage = 9

                    elif OffenseYellow2.rangePosition(self, robot_x, robot_y, 0.59, 0.67, 0.01, 0.01):
                        self.trap_stage = 9
        ...
        # Support Trap Ball
            ...
            if trap_stage == 1:
                ...
                if not OffenseYellow2.rangePosition(self, robot_x, robot_y, dest_x, dest_y, 0.01, 0.01):
                    left_speed, right_speed = OffenseYellow2.toPoint(self, left_speed, right_speed, dest_x, dest_y,
                                                                   robot_x,
                                                                   robot_y, robot_angle, spin_speed, angle_range)
                else:
                ...
                if OffenseYellow2.rangePosition(self, offense1_x, offense1_y, -0.47, -0.67, 0.005, 0.005):
                    shoot_angle = 0
                    dest_x, dest_y = -0.5715, -0.595
                    if not OffenseYellow2.rangePosition(self, robot_x, robot_y, dest_x, dest_y, 0.005, 0.005):
                        left_speed, right_speed = OffenseYellow2.toPoint(self, left_speed, right_speed, dest_x, dest_y,
                ...
                if OffenseYellow2.rangePosition(self, offense1_x, offense1_y, -0.57, 0.57, 0.005, 0.005):
                    shoot_angle = 90
                    dest_x, dest_y = -0.495, 0.672
                    if not OffenseYellow2.rangePosition(self, robot_x, robot_y, dest_x, dest_y, 0.005, 0.005):
                        left_speed, right_speed = OffenseYellow2.toPoint(self, left_speed, right_speed, dest_x, dest_y,
        ...
        temp_pred_distance = OffenseYellow2.getDistance(self, robot_x, robot_y, temp_pred_x, temp_pred_y)
        if not run_pass and ball_y < 0 and -0.23 < robot_x < 0.23 and robot_y < 0 and (
                ...
                temp_pred_distance < distance_range and temp_pred_y < robot_y) or (
                        OffenseYellow2.getDistance(self, robot_x, robot_y, self.last_ball_x,
    ...
    def toPoint(self, left_speed, right_speed, dest_x, dest_y, robot_x, robot_y, robot_angle, spin_speed,
                angle_range):
        closest_angle, absolute_angle = OffenseYellow2.coords(self, robot_angle, robot_x, robot_y, dest_x, dest_y)
        if not OffenseYellow2.rangeAngle(self, robot_angle, closest_angle, angle_range):
            left_speed, right_speed = OffenseYellow2.spin(self, left_speed, right_speed, robot_angle, closest_angle,
                                                    spin_speed)
        else:
            if self.team == "B":
                left_speed, right_speed = OffenseYellow2.moveStraight(self, robot_angle, absolute_angle, -10)
            else:
                left_speed, right_speed = OffenseYellow2.moveStraight(self, robot_angle, absolute_angle, 10)
        return left_speed, right_speed
    ...
    def coords(self, robot_angle, robot_x, robot_y, dest_x, dest_y):
        ...
        closest_angle, closest_difference = OffenseYellow2.direction(self, robot_angle, absolute_angle,
    ...
    def spin(self, left_speed, right_speed, robot_angle, angle, spin_speed):
        closest_angle, closest_difference = OffenseYellow2.direction(self, robot_angle, angle, (angle + 180) % 360)
    ...
    def moveStraight(self, robot_angle, target_angle, move_speed):
        closest_angle, closest_difference = OffenseYellow2.direction(self, target_angle, robot_angle,
                                                               (robot_angle + 180) % 360)
    ...
    def rangeAngle(self, angle, target_angle, angle_range=15, angle_left_range=None, angle_right_range=None,
        ...
        if both_sides:
            closest_angle, closest_difference = OffenseYellow2.direction(self, angle, target_angle,
                                                                   (target_angle + 180) % 360)
        else:
            closest_angle, closest_difference = OffenseYellow2.direction(self, angle, target_angle, target_angle)
    ...
    def findAttackPoint(self, point_x, ball_x, ball_y, last_ball_x, last_ball_y, offset_front=0.0,
        ...
        main_x, main_y = OffenseYellow2.findPoint(self, point_x, ball_x, ball_y, last_ball_x, last_ball_y)
