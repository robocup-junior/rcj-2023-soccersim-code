...
class DefenseBlue:
    ...
    def run_program(self, robot, data, lop1, lop2, roles):
        ...
        self.pred_n = DefenseBlue.rescale(self, ball_dist, 30, 50, 0, 0.8)
        ...
        # Mode Switching
        if abs(ball_pos_diff['y']) <= 0.0045 and ball_pos['x'] > 0.44 and (ball_pos['y'] > 0.35 or ball_pos['y'] < -0.35):
        ...
        if (self.mode == wall_side_defense_mode or self.mode == diagonal_left_defense_mode or self.mode == diagonal_right_defense_mode) and (ball_dist > 0.161 and abs(ball_pos_diff['y']) < 0.001):
        ...
        if self.mode == 5:
            left_speed, right_speed = DefenseBlue.toPoint(self, left_speed, right_speed, 0.7, 0,
        ...
        # Wall Defense In Penalty Area (With Deflection)
            ...
            angle = DefenseBlue.direction(self, robot_angle, 90, 270)
            if (robot_y > -0.21 and robot_y < 0) and (robot_x > 0.7) and (
                ...
                angle = DefenseBlue.direction(self, robot_angle, 0, 180)
                left_speed, right_speed = DefenseBlue.move(self, robot_angle, -10, -10, angle,
                                                       180, False)
                ...
                if robot_angle > angle + angle_range or robot_angle < angle - angle_range:
                    left_speed, right_speed = DefenseBlue.spin(self, robot_angle, angle, left_speed,
                                                           right_speed,
                                                           spin_speed, angle_range)
                else:
                    left_speed, right_speed = DefenseBlue.move(self, robot_angle, 10, 10, angle,
                                                           270, False)
                    left_speed, right_speed = DefenseBlue.border_x(self, robot_angle, robot_pos['x'],
                                                               left_speed,
        ...
        # Wall Defense Outside Of Penalty Area (With Going To Wall Defense Point)
            ...
            if (robot_y <= 0.23 or robot_y > 0.38) and ball_y >= 0.28:
                left_speed, right_speed = DefenseBlue.toPoint(self, left_speed, right_speed, 0.67, 0.3,
                                                          robot_pos['x'], robot_pos['y'], robot_angle,
                                                          spin_speed, angle_range)
        ...
        # Diagonal Ball Tracking On The Robot's Left Side (Blue)
            ...
            if (robot_x > 0.65 or robot_x < 0.57):
                left_speed, right_speed = DefenseBlue.toPoint(self, left_speed, right_speed, 0.64, 0.28,
                                                          robot_pos['x'], robot_pos['y'], robot_angle,
                                                          spin_speed, angle_range)
            else:
                angle = DefenseBlue.direction(self, robot_angle, 60, 240)
                if robot_angle > angle + angle_range or robot_angle < angle - angle_range:
                    left_speed, right_speed = DefenseBlue.spin(self, robot_angle, angle, left_speed, right_speed,
                                                           spin_speed,
                                                           angle_range)
                else:
                    solution_x, solution_y = DefenseBlue.findPoint(self, robot_angle, robot_x, robot_y, ball_x,
                                                               ball_pos_pred['y'],
                                                               0.745, 0)
                    left_speed, right_speed = DefenseBlue.chase(self, robot_angle, robot_pos['y'],
                                                            solution_y)
                    left_speed, right_speed = DefenseBlue.border_y(self, robot_angle, robot_pos['y'],
        ...
        # Diagonal Ball Tracking On The Robot's Right Side (Blue)
            ...
            if (robot_x > 0.65 or robot_x < 0.57):
                ...
                angle = DefenseBlue.direction(self, robot_angle, 120, 300)
                if robot_angle > angle + angle_range or robot_angle < angle - angle_range:
                    left_speed, right_speed = DefenseBlue.spin(self, robot_angle, angle, left_speed,
        ...
        # Regular Ball Tracking In A Straight Line
            ...
            if abs(0.56 - robot_pos['x']) > self.goalline_range:
                ...
                left_speed, right_speed = DefenseBlue.toPoint(self, left_speed, right_speed, 0.56, temp_ball_pos_y,
                                                              robot_pos['x'], robot_pos['y'], robot_angle,
                                                              spin_speed, angle_range)
    ...
    def toPoint(self, left_speed, right_speed, dest_x, dest_y, robot_pos_x, robot_pos_y, robot_angle, spin_speed,
                angle_range):
        angle, absolute_angle = DefenseBlue.coords2(self, robot_angle, robot_pos_x, robot_pos_y,
                                        dest_x,
                                        dest_y)
        if robot_angle > angle + angle_range or robot_angle < angle - angle_range:
            left_speed, right_speed = DefenseBlue.spin(self, robot_angle, angle, left_speed,
    ...
    def move(self, robot_angle, left_run, right_run, relative, absolute, straight):
        if straight == True:
            ...
            angle = DefenseBlue.direction(self, robot_angle, relative, absolute)
            if angle == absolute:
                left_speed = left_run
                right_speed = right_run
            else:
    ...
    def toPoint2(self, left_speed, right_speed, dest_x, dest_y, robot_pos_x, robot_pos_y, robot_angle):
        spin_speed = 6
        angle_range = 15
        angle, absolute_angle = DefenseBlue.coords2(self, robot_angle, robot_pos_x, robot_pos_y, dest_x, dest_y)
        if robot_angle > angle + angle_range or robot_angle < angle - angle_range:
            left_speed, right_speed = DefenseBlue.spin(self, robot_angle, angle, left_speed,
                                           right_speed,
                                           spin_speed, angle_range)
    ...
    def chase(self, robot_angle, robot_y, ball_y):
        if ball_y < robot_y:
            left_speed, right_speed = DefenseBlue.move(self, robot_angle, -7, -7, 0, 0, True)
        elif ball_y > robot_y:
            left_speed, right_speed = DefenseBlue.move(self, robot_angle, 7, 7, 0, 0, True)
        else:
            left_speed, right_speed = DefenseBlue.move(self, robot_angle, 0, 0, 0, 0, True)
        # print("Chase")
        return left_speed, right_speed
    ...
    def border_x(self, robot_angle, robot_x, left_speed, right_speed, limit1, limit2, given_range, return_speed=10):
        if limit2 < limit1:
            ...
            return left_speed, right_speed
        if robot_x < limitA:
            left_speed, right_speed = DefenseBlue.move(self, robot_angle, return_speed, return_speed, 0, 0, True)
        elif robot_x > limitB:
            left_speed, right_speed = DefenseBlue.move(self, robot_angle, -return_speed, -return_speed, 0, 0, True)
